...
function formatWeekDisplay(d) {
    const monday = new Date(getWeekId(d) + 'T00:00:00'); 
...
export async function initializeWellness(userId, onWellnessDataUpdate) {
    ...
    unsubscribeDailyWellness = onSnapshot(dailyWellnessRef, (docSnap) => {
        if (docSnap.exists()) {
            ...
            dailyWellnessData = initialDailyData; 
        }
...
async function loadWellnessForDate(date, onWellnessDataUpdate) {
    ...
    await initializeWellnessData(wellnessDataRef); 
...
// --- SECURE BACKEND CALL 1: Symptom Checker ---
async function handleSymptomCheck() {
    const symptomText = symptomInput.value.trim();
    if (!symptomText) return;
    
    symptomLoader.classList.remove('hidden'); 
    symptomBtnText.textContent = 'Analyzing...'; 
    symptomCheckBtn.disabled = true; 
    symptomResponseEl.innerHTML = '';
    ...
    try {
        ...
        if (symptomTrackerCollectionRef) {
            await addDoc(symptomTrackerCollectionRef, { symptom: symptomText, aiResponse: data, createdAt: serverTimestamp() });
        }
    } catch (error) {
        ...
        symptomBtnText.textContent = 'Get Suggestions'; 
        symptomCheckBtn.disabled = false;
    }
}
...
function updateDynamicContent() {
    ...
    wellnessGlow.className = "wellness-glow"; 
    if (displayWeek <= 13) {
        wellnessGlow.classList.add('glow-green'); 
    } else if (displayWeek <= 27) {
        wellnessGlow.classList.add('glow-lavender'); 
    } else {
        wellnessGlow.classList.add('glow-blue'); 
    }
}
...
// --- SECURE BACKEND CALL 2: Add Supplement ---
async function handleAddSupplement() {
    const userSupp = newSupplementInput.value.trim(); if (!userSupp) return;
    
    supplementApiLoader.classList.remove('hidden'); 
    addSupplementText.textContent = 'Checking...'; 
    addSupplementBtn.disabled = true; 
    supplementApiFeedback.classList.add('hidden');
    ...
    const userQuery = `Evaluate this supplement for pregnancy: "${userSupp}"`; 
    ...
    const payload = {
     ...
     try {
        ...
        showApiFeedback("Sorry, I couldn't verify that right now.", 'error', supplementApiFeedback); 
    } finally { 
        supplementApiLoader.classList.add('hidden'); 
        addSupplementText.textContent = 'Check & Add Supplement'; 
        addSupplementBtn.disabled = false; 
    }
}

function openNutritionHistoryModal() {
    nutritionHistoryCurrentDate = new Date(); 
    populateNutritionHistory(nutritionHistoryCurrentDate);
...
async function populateNutritionHistory(date) {
    ...
    days.forEach((dayKey, index) => {
        ...
        const mondayOfRelevantWeek = new Date(getWeekId(date) + 'T00:00:00'); 
...
export async function generateAllWellnessTips() {
    ...
    try {
        ...
        const today = new Date();
        const diffTime = Math.max(0, today.getTime() - pregnancyStartDate.getTime());
        const pregnancyWeek = Math.floor(diffTime / (1000 * 60 * 60 * 24 * 7)) + 1; 
...
function openEditDayModal(dayKey) {
    ...
    const mondayOfRelevantWeek = new Date(getWeekId(wellnessHistoryCurrentDate) + 'T00:00:00'); 
    const dayDate = new Date(mondayOfRelevantWeek);
