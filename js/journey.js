...
function setupEventListeners() {
    ...
    elements.aiGenerateTodosBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            const data = JSON.parse(result.data.candidates[0].content.parts[0].text);
    ...
    elements.aiWishForm.addEventListener('submit', async (e) => {
        ...
        try {
            const result = await callGeminiWithBackoff(payload);
            let jsonString = result.data.candidates[0].content.parts[0].text;
    ...
    elements.aiSummarizeReflectionsBtn.addEventListener('click', async () => {
        ...
        try {
            const result = await callGeminiWithBackoff(payload);
            elements.aiSummaryContent.textContent = result.data.candidates[0].content.parts[0].text;
        } catch (error) {
...
async function generateNames(isRandom = false) {
    ...
    try {
        const result = await callGeminiWithBackoff(payload);
        if (result.data.candidates && result.data.candidates[0].content && result.data.candidates[0].content.parts[0].text) {
             const namesData = JSON.parse(result.data.candidates[0].content.parts[0].text);
