 ...
 const customCategoryInput = document.getElementById('custom-category-input'); // This might be for wishes
 ...
 const customTodoCategoryInput = document.getElementById('custom-todo-category-input'); // This is for todos
 ...
 async function requestNotificationPermission() {
     ...
     notificationPermissionGranted = false; // Explicitly set if denied or not supported
     return false;
 }
 // --- End NEW ---
 ...
 function scheduleNotification(todo) {
     ...
     // Ensure todo has both date and time
     if (!todo.date || !todo.time || todo.completed) {
         return; // No date/time or already completed
     }
     ...
     try {
         ...
         if (delay > 0) {
             ...
             console.log(`Scheduling notification for todo "${todo.text}" in ${delay / 1000} seconds.`);
             const timeoutId = setTimeout(() => {
                 console.log("Showing notification for:", todo.text);
                 // Check permission again right before showing, in case it changed
                 if (Notification.permission === 'granted') {
                     new Notification('Pregnancy Planner Reminder:', {
                         body: todo.text,
                         icon: './assets/icons/icon-192x192.png' // Optional: Add an icon URL
                     });
                 }
                 // Remove from scheduled list after it fires
             ...
             scheduledNotifications[todo.id] = timeoutId;
              ...
              // If the time is in the past, clear any old notification ID if it exists
              clearScheduledNotification(todo.id);
         }
     } catch (error) {
         console.error("Error parsing date/time or scheduling notification for todo:", todo.id, error);
          // Clear any potentially invalid notification ID
          clearScheduledNotification(todo.id);
     }
 }
 // --- End NEW ---
 ...
 function clearScheduledNotification(todoId) {
     if (scheduledNotifications[todoId]) {
         ...
         delete scheduledNotifications[todoId]; // Remove from tracking
     }
 }
 // --- End NEW ---
 ...
 function renderTodos(todos) {
     ...
     todos.forEach(todo => {
         ...
         const toggleTodo = async () => {
              ...
              const parentDiv = item; // the todo-item div
              if(isNowCompleted) {
              ...
              // Update todo object locally for notification logic
              todo.completed = isNowCompleted;
 
              // Then update Firestore
              await updateDoc(todoDocRef, { completed: isNowCompleted });
 
              // --- NEW: Clear notification if completed ---
              if (isNowCompleted) {
                 clearScheduledNotification(todo.id);
              } else { // If it's now *not* completed, reschedule if needed
                  scheduleNotification(todo); // Pass the locally updated todo object
              }
              // --- End NEW ---
         };
         ...
         item.querySelector('.delete-todo-btn').addEventListener('click', async () => {
             ...
             // --- NEW: Clear notification on delete ---
             clearScheduledNotification(todo.id);
             // --- End NEW ---
         });
 ...
 function loadTodos() {
     ...
     unsubscribeTodos = onSnapshot(q, (snapshot) => {
         ...
         // --- NEW: Schedule notifications for loaded todos ---
         currentTodos.forEach(todo => {
             if (!todo.completed) { // Only schedule for incomplete tasks
                 scheduleNotification(todo);
             } else {
                 clearScheduledNotification(todo.id); // Ensure completed ones are cleared
             }
         });
         // --- End NEW ---
     });
 ...
 function setupEventListeners() {
     addTodoBtn.addEventListener('click', async () => {
         ...
         // --- NEW: Request permission before adding ---
         let hasPermission = notificationPermissionGranted;
         if (newTodoDate.value && newTodoTime.value && Notification.permission !== 'granted' && Notification.permission !== 'denied') {
             // Ask for permission only if date/time is set and not already granted/denied
              hasPermission = await requestNotificationPermission();
              // Note: User might deny here. Notification won't schedule if hasPermission is false.
         }
         // --- End NEW ---
         ...
         // --- NEW: Schedule notification after adding ---
         if (notificationPermissionGranted) { // Check the updated permission status
     ...
     addWishBtn.addEventListener('click', async () => {
         ...
         if (category === 'Custom') {
             category = customCategoryInput.value.trim(); // Use the correct custom input for wishes
         }
         ...
         customCategoryInput.value = ''; // Clear wish custom input
         newWishCategory.value = 'Baby Care';
         customCategoryInput.classList.add('hidden'); // Hide wish custom input
     });
     ...
     newWishCategory.addEventListener('change', () => {
         // Use the correct custom input ID for wishes
         if (newWishCategory.value === 'Custom') {
             customCategoryInput.classList.remove('hidden'); // Assuming this is the wish custom input
         } else {
             customCategoryInput.classList.add('hidden');
         }
     });
     ...
     newTodoCategory.addEventListener('change', () => {
         // Use the correct custom input ID for todos
 ...
 async function handleSaveTodo() {
     ...
     // --- NEW: Reschedule notification on edit ---
     // Fetch the updated todo data to ensure we have the latest info including completion status
     ...
     if (updatedTodoSnap.exists()) {
         const updatedTodoFullData = { id: activeTodoId, ...updatedTodoSnap.data() };
          // Request permission again if needed (e.g., if date/time was just added and permission wasn't granted yet)
          if (updatedTodoFullData.date && updatedTodoFullData.time && !updatedTodoFullData.completed && Notification.permission !== 'granted' && Notification.permission !== 'denied') {
             await requestNotificationPermission();
          }
         scheduleNotification(updatedTodoFullData); // Use the full data including completion status
     }
     // --- End NEW ---
