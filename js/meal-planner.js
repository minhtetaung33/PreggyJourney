...
function renderMealPlan(data) {
    ...
    for (let i = 0; i < 7; i++) {
        ...
        weekDates.push(date.getUTCDate()); 
    }
...
async function handleSaveEdit(key, oldMeal) {
    ...
    try {
        ...
        if (data.isSuitable) {
            const currentMeals = [...(dynamicMealOptions[key] || [])];
            if (currentMeals.includes(data.mealName) && data.mealName !== oldMeal) { 
                showApiFeedback(`"${data.mealName}" is already on the list.`, 'warning', editApiFeedback); 
                return; 
            }
        ...
        console.error("Gemini API call failed during edit:", error); 
        showApiFeedback("Sorry, I couldn't verify that meal right now. Please try again.", 'error', editApiFeedback); 
    } finally { 
        editApiLoader.classList.add('hidden'); 
        editMealText.textContent = 'Check & Save'; 
...
async function handleAddMeal() {
    ...
    try {
        ...
        showApiFeedback("Sorry, I couldn't verify that meal right now. Please try again.", 'error', apiFeedback); 
    } finally { 
        apiLoader.classList.add('hidden'); 
        addMealText.textContent = 'Check & Add Meal'; 
...
// --- SECURE BACKEND CALL 3: Generate AI Meal Plan ---
